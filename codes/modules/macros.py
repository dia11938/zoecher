...
class Macros:
	...
	def init(self,gch):
		self.gmacrolist = eval(read_file("history/macros.db"))
		self.gaccesslist = eval(read_file("history/macroaccess.db"))

		mcrfile='history/'+gch+'/macros.db'
		mcracfile='history/'+gch+'/macroaccess.db'
	...
	def flush(self):
		for x in self.macrolist.keys():
			write_file('history/'+x+'/macros.db', str(self.macrolist[x]))
		for x in self.accesslist.keys():
			write_file('history/'+x+'/macroaccess.db', str(self.accesslist[x]))
