...
def create_file(gch='',file=''):
	...
	if os.path.exists(pthf):
		...
		try:
			...
			return 0
def init_warfiles():
	create_file(file='drjabber_green.db')
	create_file(file='drjabber_red.db')
	create_file(file='otake.db')
	create_file(file='otake_serv.db')
	create_file(file='spamserv.db')
...
zooPyVer = 'v%s [%s]' % (sys.version.split(' (')[0],sys.version.split(')')[0].split(', ')[1])
	...
	for possibility in possibilities:
		if possibility[-3:].lower() == '.py':
			try:
				...
				pass
	if invalid_plugins:
		print '\nFailed 2 Load',len(invalid_plugins),'Orders:'
	else:
		print '\n - Success!'
		...
		if iq.getTags('query', {}, xmpp.NS_VERSION):
			if not BOT_VER['botver']['os']:
				...
				if os.name=='nt':
					...
					osname2.close
				pyver = sys.version
				BOT_VER['botver']['os'] = osver + osver2 +' | ' + 'Python %s | Twisted: %s' % (zooPyVer,twisted.__version__)
			result = iq.buildReply('result')
			query = result.getTag('query')
			query.setTagData('name', 'zOeCher')
			...
			items=[]
			ids=[]
			ids.append({'category':'jabber','type':'bot','name':'zOeCher'})
